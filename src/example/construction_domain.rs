...
fn create_initial_tasks() -> (Rc<Task<u32>>, Rc<Task<u32>>, Rc<Task<u32>>, Rc<Task<u32>>) {
    ...
    let t3 = Task::Compound(CompoundTask::new(
        "Construct".to_string(),
        Vec::from([Method::new(
            "method-01".to_string(),
            HTN::new(
...
fn decomposition_tasks<T: Eq + Hash + Clone>() -> (Task<T>, Task<T>, Task<T>, Task<T>, Task<T>) {
    let empty = HashSet::new();
    let t1 = Task::Primitive(PrimitiveAction::new(
...
pub fn create_state_problem_instance() -> HTN<String> {
    let t1 = Task::Primitive(PrimitiveAction::new(
        "ObtainPermit".to_string(),
        HashSet::new(),
        HashSet::from(["obtained_permit".to_string()]),
        HashSet::new(),
    ));
    let t2 = Task::Primitive(PrimitiveAction::new(
        "HireBuilder".to_string(),
    ...
    let (t5, t6, t7, t8, t9) = decomposition_tasks::<String>();
    let t3 = Task::Compound(CompoundTask::new(
        "Construct".to_string(),
        Vec::from([Method::new(
            "method-01".to_string(),
    ...
    let t4: Task<String> = Task::Primitive(PrimitiveAction::new(
        "PayBuilder".to_string(),
        HashSet::new(),
        HashSet::new(),
        HashSet::new(),
    ...
    let alpha = HashMap::from(
        [(1, Rc::new(t1)), (2, Rc::new(t2)), (3, Rc::new(t3)), (4, Rc::new(t4))]
    );
    let orderings: Vec<(u32, u32)> = Vec::from([(1, 3), (2, 3), (3, 4)]);
    let t = HashSet::from([1,2,3,4]);
    HTN::new(t, orderings, alpha)
}